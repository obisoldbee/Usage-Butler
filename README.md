...
0.3.0（3）新增原生上下行分区独立刻度、五档范围、共享游标、源侧两小时有界历史与各方向独立累计起点。0.3.1（4）修复普通应用读取系统字节数的 32 位截断，改用公开 IFMIB 的完整计数；历史提醒只针对所选范围内的实际中断，裸 Tab 可从网络页返回额度。0.3.2（5）修正时间范围按钮的完整点击区域，把接口选择和统计说明移到独立网络设置页，取消主面板折叠和游标导致的高度变化。

正式版仅观察单个系统网络接口，尚无真实按应用/目标采集或连接阻断。完整应用交互仅在明确标识的 Debug 演示中出现，Release 不包含演示应用。实现和验证边界见[网络交付说明](docs/network-redesign-brief.md)及[评审入口](docs/network-v2-review-guide.md)。
