...
网络页现可显示单个系统网络接口的上下行速率、趋势、会话累计与覆盖状态，支持自动识别当前观察连接。**尚不支持真实按应用/目标统计、完整应用详情、网络导出或连接阻断**，不是完整防上传防护。

当前共享纵轴、时间选项布局、混合采样下的历史散点和累计起点仍有已知问题；下一版采用上传/下载分区独立纵轴。完整状态与原型交付要求见 [网络整改说明](docs/network-redesign-brief.md)。Debug 自渲染验收不等于完整用户交互验收；退出路径另有待复验问题。本次为源码更新，未发布新的 GitHub 安装包。
